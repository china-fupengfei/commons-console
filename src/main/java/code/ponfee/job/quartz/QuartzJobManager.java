...
public class QuartzJobManager implements InitializingBean, DisposableBean {
    ...
    public void addOrUpdateJob(SchedJob job) {
        ...
        if (nextFireTime == null) {
            logger.error("This quartz job cannot be fire: {}.", Jsons.toJson(job));
            return;
        }
        ...
        try {
            ...
            if (isNewJobDetail) {
                ...
                boolean recovery = ObjectUtils.orElse(job.getRecoverySupport(), false);
                ...
                prevJob = ObjectUtils.orElse((SchedJob) jobDetail.getJobDataMap().get(SCHED_JOB_DATA), job);
            }
