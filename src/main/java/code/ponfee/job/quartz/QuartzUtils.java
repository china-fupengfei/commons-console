...
public class QuartzUtils {
    ...
    private static final Set<String> PARAMS = ImmutableSet.of(PARAM_STEP, PARAM_ORIGIN);
    ...
    public static List<Date> listNextFireTime(TriggerType trigger, String schedule,
        ...
        switch (trigger) {
            ...
            default:
                ...
                start = (start == null) ? new Date() : Dates.plusSeconds(start, 1);
                while (numTimes-- > 0) {
                    result.add(start = periods.next(origin, start, step, 1).beginDate());
                }
                return result;
    ...
    public static String toCronExpression(Date date) {
        ...
        return new StringBuilder(24)
            .append(dt.getSecondOfMinute()).append(BLANK_CHAR) // second
            .append(dt.getMinuteOfHour()  ).append(BLANK_CHAR) // minute
            .append(dt.getHourOfDay()     ).append(BLANK_CHAR) // hour
            .append(dt.getDayOfMonth()    ).append(BLANK_CHAR) // day
            .append(dt.getMonthOfYear()   ).append(BLANK_CHAR) // month
            .append('?'                   ).append(BLANK_CHAR) // week
            .append(dt.getYearOfEra()     )                    // year
            .toString();
    }

    /**
     * Clone trigger if type of OperableTrigger
     * 
     * @param trigger the Trigger
     * @return a duplicate trigger argument
     */
    public static Trigger cloneTrigger(Trigger trigger) {
        if (trigger instanceof OperableTrigger) {
            trigger = (Trigger) ((OperableTrigger) trigger).clone();
        }
        return trigger;
    }
