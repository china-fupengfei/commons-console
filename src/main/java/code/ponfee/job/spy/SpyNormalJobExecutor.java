...
public class SpyNormalJobExecutor implements SpyJobExecutor {
    ...
    public void execute(SchedJob job) {
        ...
        try {
            ...
            boolean isManual = ObjectUtils.orElse(job.getManualTrigger(), false)
            ...
            if (isManual) {
                ...
                try {
                    schedJobDao.incrAndRank(HOST_IP, job.getScore() * -1);
                } catch (Exception err) {
            ...
            try {
                schedJobDao.recordLog(new SchedLog(
                    HOST_IP, result, isManual, job.getId(), job.getName(), 
