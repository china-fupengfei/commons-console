...
import static code.ponfee.commons.util.Networks.HOST_IP;
...
public class SpyJobHeartbeat {
    ...
    private static final class HeartbeatRunner extends Thread {
        ...
        public void run() {
            while (running) {
                ...
                try {
                    ...
                    if (schedJobDao.incrAndRank(HOST_IP, 0) > 0) {
                        return;
                    }
                    ...
                    for (long jobId : schedJobDao.listJobIds()) {
                        ...
                        if (job.getNextSchedTime() == null) {
                            Date nextFireTime = CommonUtils.computeFirstFireTime(job);
                        ...
                        if (tryAcquire(job, now)) {
                            schedJobDao.incrAndRank(HOST_IP, job.getScore());
        ...
        private boolean tryAcquire(SchedJob job, Date date) {
            ...
            if (schedTime.after(date)) {
                ...
                job.setLastSchedServer(HOST_IP); // 执行服务器
                job.setNextSchedTime(computeNextFireTime(job, schedTime)); // 更新下一次执行时间点
            }
            // 先获取锁再判断时间
    ...
    private static Date computeNextFireTime(SchedJob job, Date lastSchedTime) {
        return QuartzUtils.getNextFireTime(
            TriggerType.of(job.getTriggerType()), 
            job.getTriggerSched(), lastSchedTime
        );
    }
}
